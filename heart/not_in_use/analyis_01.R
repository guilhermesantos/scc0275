...
for (k in 1:10){
  ...
  tab_10fold[k, 1:2] = run_CV_TM(mod_spec = knn_wf, folds = folds, dataset = "train")$metric[1, 2:3] %>% as.matrix()
  tab_10fold[k, 3:4] = run_CV_TM(mod_spec = knn_wf, folds = folds, dataset = "test")$metric[1, 2:3] %>% as.matrix()
}
